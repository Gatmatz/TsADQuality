...
The selection rule itself (`_select_missing_indices`) is a verbatim transcription of the
...
RESULTS_DIR = _ROOT / "results" / "experiments" / "missing_mnar_tsbad"

# Same grid as the original run_missing_mnar.py
...
# ==========================================
# MISSING-POINT SELECTION
# ==========================================
def _select_missing_indices(values, fraction, mechanism, rng):
...
def process_single_job(job_args):
    ...
    (file_path, mechanism, fraction, seed, model_names, window_mode) = job_args
    file_name = os.path.basename(file_path)
    condition_name = _cond_name(mechanism, fraction)
    ...
    try:
        ...
        # 2. Choose the missing points (skip for the clean anchor). Whole ROWS are dropped, so
        # every channel stays aligned; the mechanism weights positions by channel 0.
        if mechanism is None:
            nan_mask = np.zeros(n, dtype=bool)
        else:
            rng = np.random.default_rng(seed)
            missing_idx = _select_missing_indices(clean_data[:, 0], fraction, mechanism, rng)
            nan_mask = np.zeros(n, dtype=bool)
        ...
        for model_name in model_names:
            base = {'file': file_name, 'mechanism': mechanism, 'fraction': fraction,
...
def compute_summary(df_results, output_path):
    ...
    if ok.empty:
        ...
        return
    keys = [k for k in ['condition', 'mechanism', 'fraction', 'model'] if k in ok.columns]
    rows = []
    for name, g in ok.groupby(keys, dropna=False):
        row = dict(zip(keys, name if isinstance(name, tuple) else (name,)))
        row['n_runs'] = len(g)
        for extra in ('actual_missing_rate', 'n_lost_anomalies', 'n_lost_normal',
                      'pct_anomalies_lost', 'n_kept'):
...
# ==========================================
def main():
    ap = argparse.ArgumentParser(description='Missing values MCAR vs MNAR (point) — TSB-AD (350)')
    ...
    conditions += [(mech, f) for mech in mechanisms for f in fractions]

    print(f"\n{'='*66}\n  Missing values — MCAR vs MNAR (point) — TSB-AD (full 350)\n{'='*66}")
    ...
    print(f"{'='*66}")
    print(f"Files: {len(file_paths)} | fractions: {fractions}")
    ...
    for fp in file_paths:
        ...
        for (mech, frac) in conditions:
            cond = _cond_name(mech, frac)
            # The clean anchor is deterministic: one seed reproduces it exactly.
            n_seeds = 1 if mech is None else N_SEEDS
            for seed in range(n_seeds):
                need = [m for m in args.models if f"{fn}|{cond}|{seed}|{m}" not in completed]
                if need:
                    jobs.append((fp, mech, frac, seed, need, args.window_mode))
    ...
    if jobs:
        ...
        with ProcessPoolExecutor(max_workers=args.workers) as ex:
            ...
            for fut in tqdm(as_completed(futs), total=len(jobs), desc="missing(mcar-vs-mnar)"):
                res = fut.result()
