...
RESULTS_DIR = _ROOT / "results" / "experiments" / "freeze_tsbad"

# Same grid as the original TSB-UAD run_freeze.py
...
#
# This matters more for freeze than for any other corruption: a frozen block is a flat line,
# which is exactly what the ACF periodicity estimator is worst at. Leaving the window free
# would confound "detector degraded" with "we measured it with a different ruler".
...
def process_single_job(job_args):
    ...
    (file_path, fraction, num_stucks, seed, model_names, window_mode) = job_args
    file_name = os.path.basename(file_path)
    condition_name = _cond_name(fraction, num_stucks)

    try:
        from TSB_AD.evaluation.metrics import get_metrics
        from TSB_AD.utils.slidingWindows import find_length_rank

        # 1. Load (official TSB-AD convention). reset_index is REQUIRED: inject_sensor_stuck
        #    walks the block with `.loc[start_idx + i]`, i.e. label arithmetic, so the frame
        #    must carry a clean RangeIndex. No-op when the file has no NaN rows.
        ...
        if fraction is None:
            ...
            n_value_changed = int(
                (df_work[value_col].to_numpy() != df[value_col].to_numpy()).sum())
        ...
        for model_name in model_names:
            base = {'file': file_name, 'fraction': fraction, 'num_stucks': num_stucks,
                    'stuck_length': stuck_length, 'seed': seed, 'model': model_name,
                    'condition': condition_name, 'n_points': n,
                    'n_corrupted': n_corrupted,
                    'corrupted_frac': round(n_corrupted / n, 6) if n else 0.0,
                    'n_value_changed': n_value_changed,
                    'metric_window': int(sliding_window)}
            try:
...
def compute_summary(df_results, output_path):
    ...
    if ok.empty:
        ...
        return
    keys = ['condition', 'fraction', 'num_stucks', 'model']
    ...
    for name, g in ok.groupby(keys, dropna=False):
        ...
        row['n_runs'] = len(g)
        # Nominal vs realised freeze: `mean_stuck_length` is what we asked for,
        # `mean_corrupted_frac` is what the series actually got.
        if 'stuck_length' in g.columns:
            row['mean_stuck_length'] = round(g['stuck_length'].mean(), 1)
        if 'corrupted_frac' in g.columns:
            row['mean_corrupted_frac'] = round(g['corrupted_frac'].mean(), 4)
        for c in cols:
...
def main():
    ap = argparse.ArgumentParser(description='Freeze (sensor-stuck) robustness — TSB-AD (350)')
    ...
    ap.add_argument('--no-clean', action='store_true', help='Skip the clean (no-freeze) anchor condition')
    ...
    # (fraction, num_stucks); None fraction = the clean anchor
    conditions = [] if args.no_clean else [(None, None)]
    conditions += [(f, ns) for f in fractions for ns in num_stucks_list]

    print(f"\n{'='*60}\n  Freeze (Sensor Stuck) Robustness — TSB-AD (full 350)\n{'='*60}")
    print(f"Files: {len(file_paths)} | fractions: {fractions} | num_stucks: {num_stucks_list}")
    ...
    for fp in file_paths:
        ...
        for (frac, ns) in conditions:
            ...
            for seed in range(N_SEEDS):
                ...
                if need:
                    jobs.append((fp, frac, ns, seed, need, args.window_mode))
    ...
    if jobs:
        ...
        with ProcessPoolExecutor(max_workers=args.workers) as ex:
            ...
            for fut in tqdm(as_completed(futs), total=len(jobs), desc="freeze"):
                res = fut.result()
