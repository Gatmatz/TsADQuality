...
RESULTS_DIR = _ROOT / "results" / "experiments" / "spikes_tsbad"
...
def process_single_job(job_args):
    ...
    (file_path, fraction, multiplier, sequential, seq_len, seed, model_names, window_mode) = job_args
    file_name = os.path.basename(file_path)
    condition_name = _cond_name(fraction, multiplier, sequential, seq_len)
    ...
    try:
        ...
        for model_name in model_names:
            base = {'file': file_name, 'fraction': fraction, 'multiplier': multiplier,
                    'sequential': sequential, 'sequence_length': seq_len, 'seed': seed,
...
def compute_summary(df_results, output_path):
    ...
    if ok.empty:
        ...
        return
    keys = ['condition', 'fraction', 'multiplier', 'sequential', 'sequence_length', 'model']
...
def main():
    ap = argparse.ArgumentParser(description='Spike-error robustness — TSB-AD (350)')
    ...
    ap.add_argument('--no-clean', action='store_true', help='Skip the clean (no-spike) anchor condition')
    ...
    # (fraction, multiplier, sequential, seq_len); None fraction = the clean anchor
    conditions = [] if args.no_clean else [(None, None, False, 0)]
    conditions += [(f, m, s, sl) for f in fractions for m in multipliers for (s, sl) in modes]

    print(f"\n{'='*60}\n  Spike Robustness — TSB-AD (full 350)\n{'='*60}")
    print(f"Files: {len(file_paths)} | fractions: {fractions} | multipliers: {multipliers}")
    print(f"Modes: {modes} | clean anchor: {not args.no_clean} | conditions: {len(conditions)}")
    ...
    for fp in file_paths:
        ...
        for (frac, mult, seq, sl) in conditions:
            cond = _cond_name(frac, mult, seq, sl)
            for seed in range(N_SEEDS):
                need = [m for m in args.models if f"{fn}|{cond}|{seed}|{m}" not in completed]
                if need:
                    jobs.append((fp, frac, mult, seq, sl, seed, need, args.window_mode))
    ...
    if jobs:
        ...
        with ProcessPoolExecutor(max_workers=args.workers) as ex:
            ...
            for fut in tqdm(as_completed(futs), total=len(jobs), desc="spikes"):
                res = fut.result()
