...
Optimal_Det_HP = Optimal_Uni_algo_HP_dict['POLY']
...
def process_file(filename):
    ...
    try:
        ...
        # 3. Εκτέλεση POLY από τον wrapper
        output = run_Unsupervise_AD('POLY', data, **Optimal_Det_HP)
...
def main():
    ...
    print(f'Starting POLY for {len(file_list)} files...')
    ...
    out_dir = os.path.join(project_root, 'results', 'baselines', 'run_POLY_350')
    os.makedirs(out_dir, exist_ok=True)
    out_file = os.path.join(out_dir, 'results_final_tsb_ad.csv')
    df_results.to_csv(out_file, index=False)
    
    print('\n=======================================')
    print(f'ΤΕΛΙΚΑ ΑΠΟΤΕΛΕΣΜΑΤΑ POLY ΓΙΑ {len(df_results)} ΑΡΧΕΙΑ')
    print('=======================================')
