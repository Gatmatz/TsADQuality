...
Optimal_Det_HP = Optimal_Uni_algo_HP_dict['StreamVAE']
...
def process_file(filename):
    ...
    try:
        ...
        # 4. Εκτέλεση StreamVAE από τον wrapper
        output = run_Semisupervise_AD('StreamVAE', data_train, data, **Optimal_Det_HP)
...
def main():
    ...
    print(f'Starting StreamVAE for {len(file_list)} files...')
    ...
    out_dir = os.path.join(project_root, 'results', 'baselines', 'run_StreamVAE_350')
    os.makedirs(out_dir, exist_ok=True)
    out_file = os.path.join(out_dir, 'results_final_tsb_ad.csv')
    df_results.to_csv(out_file, index=False)
    
    print('\n=======================================')
    print(f'ΤΕΛΙΚΑ ΑΠΟΤΕΛΕΣΜΑΤΑ StreamVAE ΓΙΑ {len(df_results)} ΑΡΧΕΙΑ')
    print('=======================================')
