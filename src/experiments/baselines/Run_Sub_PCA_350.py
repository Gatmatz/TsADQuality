...
Optimal_Det_HP = Optimal_Uni_algo_HP_dict['Sub_PCA']
...
def process_file(filename):
    ...
    try:
        ...
        # 2. Εύρεση παραθύρου (το Sub_PCA χρησιμοποιεί το ίδιο για eval metrics)
        slidingWindow = find_length_rank(data[:,0].reshape(-1, 1), rank=1)
        
        # 3. Εκτέλεση Sub_PCA από τον wrapper
        output = run_Unsupervise_AD('Sub_PCA', data, **Optimal_Det_HP)
...
def main():
    ...
    print(f'Starting Sub_PCA for {len(file_list)} files...')
    ...
    out_dir = os.path.join(project_root, 'results', 'baselines', 'run_Sub_PCA_350')
    os.makedirs(out_dir, exist_ok=True)
    out_file = os.path.join(out_dir, 'results_final_tsb_ad.csv')
    df_results.to_csv(out_file, index=False)
    
    print('\n=======================================')
    print(f'ΤΕΛΙΚΑ ΑΠΟΤΕΛΕΣΜΑΤΑ Sub_PCA ΓΙΑ {len(df_results)} ΑΡΧΕΙΑ')
    print('=======================================')
