...
def process_file(filename):
    ...
    try:
        ...
        # 4. Εκτέλεση AutoEncoder από τον wrapper ως Unsupervised
        output = run_Semisupervise_AD('AutoEncoder_2', data, data, **Optimal_Det_HP)
...
def main():
    ...
    print(f'Starting AutoEncoder (Unsupervised) for {len(file_list)} files...')
    ...
    out_dir = os.path.join(project_root, 'results', 'baselines', 'run_AutoEncoder_Unsupervised_350')
    os.makedirs(out_dir, exist_ok=True)
    out_file = os.path.join(out_dir, 'results_final_tsb_ad.csv')
    df_results.to_csv(out_file, index=False)
    
    print('\n=======================================')
    print(f'ΤΕΛΙΚΑ ΑΠΟΤΕΛΕΣΜΑΤΑ AutoEncoder (Unsupervised) ΓΙΑ {len(df_results)} ΑΡΧΕΙΑ')
    print('=======================================')
