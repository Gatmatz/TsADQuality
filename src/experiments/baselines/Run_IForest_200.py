...
def main():
    file_list_path = os.path.join(project_root, 'results', 'tables', 'representative_subset_tsb_ad_vuspr_big_n200.csv')
    df_list = pd.read_csv(file_list_path)
    # The column in that csv is probably 'dataset' or 'file', let's check
    if 'file' in df_list.columns:
        file_list = df_list['file'].values
    else:
        file_list = df_list.iloc[:, 0].values # First column
        
    print(f'Starting IForest for {len(file_list)} files...')
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=10) as executor:
        ...
        for future in concurrent.futures.as_completed(futures):
            ...
            if res is not None:
                results.append(res)
            completed += 1
            print(f"Πρόοδος: {completed}/{len(file_list)} ολοκληρώθηκαν.", flush=True)

    df_results = pd.DataFrame(results)
    
    print('\n=======================================')
    print(f'ΤΕΛΙΚΑ ΑΠΟΤΕΛΕΣΜΑΤΑ IForest ΓΙΑ {len(df_results)} ΑΡΧΕΙΑ')
    print('=======================================')
