...
def run_single_job(file_path=None, model_names=None, out_put_path=None, mode='eval'):
    ...
    if mode == 'eval':
        ...
        all_models = {
            'IForest': IForest(n_estimators=200, random_state=42, slidingWindow=100),
        ...
        for name, model in models.items():
            try:
                ...
                # Εάν το μοντέλο επέστρεψε μικρότερο πίνακα, κάνουμε padding
                if len(score) < len(labels):
                    full_score = np.array([score[0]]*math.ceil((sliding_window-1)/2) + list(score) + [score[-1]]*((sliding_window-1)//2)) 
                else:
                    full_score = score
                                
                metrics = get_metrics(full_score, labels, slidingWindow=sliding_window, thre=50)
                file_results.append({
