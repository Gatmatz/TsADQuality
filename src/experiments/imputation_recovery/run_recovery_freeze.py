"""
Recovery for: Freeze (sensor stuck)

Same corruption as run_freeze.py, then detects constant segments
and replaces them with interpolation.

Compare recovered_AUC with corrupted AUC from:
  results/experiments/freeze/checkpoint.csv

Usage:
    python run_recovery_freeze.py --models IForest
    python run_recovery_freeze.py --test
"""
import os
import sys
import math
...
# ==========================================
# MUST MATCH run_freeze.py
# ==========================================
SUBSET_CSV = os.path.join(project_root, "results", "tables", "final_subset.csv")
RESULTS_DIR = os.path.join(project_root, "results", "experiments", "recovery_freeze")
BASELINE_CSV = os.path.join(project_root, "results", "tables", "baseline_141_iforest.csv")

FRACTIONS = [0.01, 0.05, 0.10, 0.20]
NUM_STUCKS = [1, 3, 5, 10, 20]
CLEANING_METHODS = ['linear']
MIN_CONSTANT_LENGTH = 3  # minimum consecutive identical values to detect as stuck
N_SEEDS = 1
...
def detect_and_fix_freeze(data, method, min_length=3):
    """Detect constant segments and replace with interpolation."""
    cleaned = data.copy()
    n = len(cleaned)
    ...
    while i < n:
        ...
        if run_length >= min_length:
            # Mark all except the first point as stuck
    ...
    # Replace stuck values with NaN, then interpolate
    cleaned[stuck_mask] = np.nan
    s = pd.Series(cleaned)
    if method == 'linear':
        cleaned = s.interpolate(method='linear').bfill().ffill().to_numpy('float')
    elif method == 'spline':
        cleaned = s.interpolate(method='spline', order=3).bfill().ffill().to_numpy('float')

    return cleaned


def process_single_job(job_args):
    (file_path, fraction, num_stucks, seed, model_names, baseline_dict) = job_args
    ...
    try:
        ...
        # === SAME CORRUPTION AS run_freeze.py ===
        ...
        for clean_method in CLEANING_METHODS:
            for model_name in model_names:
                try:
                    cleaned_data = detect_and_fix_freeze(
                        corrupted_data.copy(), clean_method,
                        min_length=MIN_CONSTANT_LENGTH
                    )
                    ...
                    if np.isnan(full_score).any() or len(np.unique(full_score)) <= 1:
                        results.append({
                            ...
                            'fraction': fraction, 'num_stucks': num_stucks,
                            'stuck_length': stuck_length,
...
def main():
    parser = argparse.ArgumentParser(description="Recovery for Freeze")
    ...
    if os.path.exists(checkpoint_path):
        ...
        for _, r in existing.iterrows():
            key = (r.get('file'), r.get('fraction'), r.get('num_stucks'),
    ...
    for file_path in file_paths:
        ...
        for seed in range(N_SEEDS):
            for fraction in FRACTIONS:
                for ns in NUM_STUCKS:
                    if done_results and all(
                        (file_name, fraction, ns, seed, m, cl) in done_results
                        for m in args.models for cl in CLEANING_METHODS
                    ):
                        skipped += 1
                        continue
                    jobs.append((file_path, fraction, ns, seed, args.models, baseline_dict))
    if skipped:
        print(f"[Resume] Skipped {skipped} completed jobs")

    print(f"\n{'='*60}")
    print(f"Recovery — Freeze")
    print(f"{'='*60}")
    ...
    with ProcessPoolExecutor(max_workers=args.workers) as executor:
        ...
        with tqdm(total=len(futures), desc="recovery (freeze)") as pbar:
