...
RESULTS_DIR = os.path.join(project_root, "results", "experiments", "spikes")
NOISE_TYPE = 'spikes'
...
def process_single_job(job_args):
    ...
    try:
        # 1. Load Data
        df, file_name = load_tsb_dataframe(file_path) #φορτονουμε τα data

        # 2. Add Spike Corruption
        ...
        # 3. Preprocessing
        # κανουμε z-score normalization αφου βαλουμε τα spikes, ισως προβλημα αν καναμε πριν αφου θα επηρεαζε το mean/std, 
        # τωρα τα spikes θα ειναι πιο εμφανη στα scaled δεδομενα
...
def main():
    ...
    if args.test:
        ...
        fractions = [0.05, 0.20]
        multipliers = [5.0]
        modes = [(False, 1), (True, 3)]
        print("!!! RUNNING IN TEST MODE !!!")

    n_workers = args.workers or os.cpu_count() or 1
    print(f"\n{'=' * 60}")
    print(f"  Spike Error Robustness Experiment")
    print(f"{'=' * 60}")
    ...
    if len(jobs) > 0:
        ...
        with ProcessPoolExecutor(max_workers=n_workers) as executor:
            ...
            with tqdm(total=len(jobs), desc="spikes") as pbar:
    ...
    if all_results:
        ...
        # Save per model -> point/burst -> (sequence_length)
        for model_name in args.models:
            model_df = df_all[df_all['model'] == model_name]
            if model_df.empty:
                continue

            # Point spikes (sequential == False)
    ...
    print(f"\n[Done] All spike experiments complete.")
