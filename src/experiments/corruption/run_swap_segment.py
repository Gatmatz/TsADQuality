"""
Swap Corruption — Segment Swap Experiment

Tests how swap segment LENGTH affects anomaly detection, keeping total
swapped fraction constant. Uses num_swaps to control granularity:
  - swap_length = fraction * n / (2 * num_swaps)

Same design as run_freeze.py (num_stucks) but for swap corruption.

Parameters:
  - fraction:   Total % of data swapped (0.01-0.30)
  - num_swaps:  Number of swap pairs (1-20)
  - swap_length is derived dynamically per series

Key question: "For the same amount of swap corruption,
              is it worse to have few large segment swaps or many small ones?"

Usage:
    python run_swap_segment.py --models IForest
    python run_swap_segment.py --test
"""
import os
import sys
import math
...
def run_model(model_name, X, scaled_data, sw, file_name=None):
    if model_name == 'IForest':
        ...
        clf = LOF(n_neighbors=20,algorithm='auto'); clf.fit(X); return clf.decision_scores_
    elif model_name == 'AE':
...
def process_single_job(job_args):
    (file_path, fraction, num_swaps, seed, model_names) = job_args

    file_name = os.path.basename(file_path)
    condition_name = f"frac_{fraction}_ns_{num_swaps}"
    ...
    try:
        ...
        # 2. Compute swap_length dynamically
            ...
            try:
                ...
                if np.isnan(full_score).any() or len(np.unique(full_score)) <= 1:
                    results.append({
                        ...
                        'num_swaps': num_swaps, 'swap_length': swap_length,
...
def compute_summary(df_results, output_path):
    ...
    grouped = df_success.groupby(['fraction', 'num_swaps', 'model'])
...
def main():
    ...
    print(f"\n{'=' * 60}")
    print(f"  Swap — Segment Swap Experiment")
    print(f"{'=' * 60}")
    print(f"  swap_length = fraction * n / (2 * num_swaps)")
    print(f"  Scales dynamically with series length")
    print(f"{'=' * 60}")
    print(f"Fractions:  {fractions}")
    print(f"Num Swaps:  {num_swaps_list}")
    print(f"Total Conditions: {len(fractions) * len(num_swaps_list)}")
    print(f"{'=' * 60}\n")
    ...
    for file_path in df_files:
        ...
        for frac in fractions:
            for ns in num_swaps_list:
                for seed in range(N_SEEDS):
                    condition = f"frac_{frac}_ns_{ns}"
                    needed_models = []
    ...
    if len(jobs) > 0:
        ...
        with ProcessPoolExecutor(max_workers=n_workers) as executor:
            ...
            with tqdm(total=len(jobs), desc="swap (segment)") as pbar:
    ...
    print(f"\n[Done] All segment swap experiments complete.")
