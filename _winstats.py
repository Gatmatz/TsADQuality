...
def acf_fft(x, nlags):
    ...
    acov = np.fft.irfft(X*np.conjugate(X), fsize)[:nlags+1] / n
    return acov / acov[0]
...
def find_length(data):
    ...
    base = 3
    ac = acf_fft(data, 400)[base:]
    lm = local_maxima(ac)
    if len(lm) == 0: return 100
    mx = lm[int(np.argmax(ac[lm]))]
    if mx < 3 or mx > 300: return 100
    return mx + base
...
ck = pd.read_csv("results/experiments/swap_segment/checkpoint.csv", low_memory=False)
files = sorted(ck[ck["error"].isna()]["file"].unique())
print("files:", len(files))
index = {}
for dp, _, fns in os.walk("TSB-UAD/data"):
    for fn in fns:
        if fn.endswith(".out"): index.setdefault(fn, os.path.join(dp, fn))
wins, missing = [], []
for f in files:
    p = index.get(f)
    if p is None:
