...
class PermutationInjector(BaseCorruptor):
    ...
    def inject(self, num_permutations):
        ...
        perm = self._derangement(num_permutations)
        ...
        self.record_corruption('permutation', np.arange(n), {
            'num_permutations': num_permutations,
            'segment_length': seg_len,
        })
        return self.df

    def _derangement(self, k):
        """A random permutation of range(k) with no element left in its place."""
        identity = np.arange(k)
        while True:
            permutation = self.rng.permutation(k)
