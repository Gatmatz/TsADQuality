...
class MissingValueInjector(BaseCorruptor):
    ...
    def inject(self, fraction, mechanism):
        ...
        if mechanism in ('MNAR_extreme', 'MNAR_high'):
            ...
            values = self.df.loc[target_indices, self.value_col].to_numpy()
            ...
            positions = ReproducibleOperations.choice(
                len(target_indices), size=count, replace=False, p=weights
            )
...
class BurstMissingInjector(BaseCorruptor):
    ...
    def inject(self, fraction, num_bursts, mechanism):
        ...
        if mechanism in ('MNAR_extreme', 'MNAR_high'):
            ...
            target_indices = ReproducibleOperations.choice(
                candidates, size=count, replace=False, p=weights
            )
        else:
        ...
        self.record_corruption('burst_missing', burst_array, {'fraction': fraction, 'num_bursts': num_bursts, 'burst_length': burst_length, 'mechanism': mechanism})
        return self.df
